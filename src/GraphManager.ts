...
export function initCharts() {
  ...
  WinRateChart = new Chart(
    <HTMLCanvasElement>document.getElementById("ctxWinRate"),
    {
      type: "bar",
      data: data,
      options: {
        ...
        tooltips: {
          callbacks: {
            label: function (tooltipItem, data) {
              ...
              const val: number =
                typeof tooltipItem.yLabel == "string" ? 0 : tooltipItem.yLabel;
  ...
  GamesChart = new Chart(
    <HTMLCanvasElement>document.getElementById("ctxGames"),
    {
      type: "bar",
      data: data,
      options: {
  ...
  ProgressChart1 = new Chart(
    <HTMLCanvasElement>document.getElementById("ctxProgress1"),
    {
      type: "line",
      data: data,
      options: {
  ...
  ProgressChart2 = new Chart(
    <HTMLCanvasElement>document.getElementById("ctxProgress2"),
    {
      type: "line",
      data: data,
      options: {
...
export function updateCharts() {
  const season = document.querySelector<HTMLInputElement>("select.season")
    .value;
  ...
  ROLES.forEach((role, index) => {
    ...
    for (let i = 1; i < 6; i++) {
      set.data[i] = roleStats.gamesPlayedGroup[i]
        ? roleStats.winRateGroup[i] * 100 - 50
        : 0;
    }
  });
  WinRateChart.update();
  ...
  ROLES.forEach((role, index) => {
    ...
    const entries = roleStats.enhancedEntries
      ...
      .reduce(
        ...
        { items: [], maxLength: PROGRESS_ENTRY_LENGTH }
      )
      .items.sort(sort("ASC", "sortId"));
